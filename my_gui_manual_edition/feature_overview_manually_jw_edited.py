...
class WindowShowFeatureOverview(QMainWindow):
    def __init__(self, x_pos_parent_window, y_pos_parent_window, width_parent_window):
        ...
        self.btn_show_anom_types_of_all_lines = QPushButton("Show all features within lines")
        self.btn_2_dummy = QPushButton("Dummy button 2")

        self.btn_layout.addWidget(self.btn_load_features_for_overview)
        self.btn_layout.addWidget(self.btn_show_anom_types_of_all_lines)
        ...
        self.btn_load_features_for_overview.clicked.connect(self.get_uploaded_items)
        self.btn_show_anom_types_of_all_lines.clicked.connect(self.show_anom_types_of_all_lines)
    ...
    def load_csv_file(self):
        dev_folder = "/users/joerg/repos/development"
        my_feature_gui_folder = "my_feature_overview_manual_edition"

        complete_folder_path = os.path.join(dev_folder, my_feature_gui_folder)
    ...
    def get_uploaded_items(self):
        ...
        saved_file_name = "saved_anomaly_list.txt"
        with open(saved_file_name) as f:
                anomaly_dirs = f.read().splitlines()

        self.anomaly_dirs = anomaly_dirs
    ...
    def show_anom_types_of_all_lines(self):
        print('New function of showing anom typess...')
        self.list_of_lines.clear()
        if isinstance(self.path_csv_file, tuple):
            ...
            str(self.path_csv_file)
        # IMPORTANT NOTE: Presume, that the scanning of the anomaly-dirs within the txt-File have been finished.
        # --> self.anomaly_dirs..
        # Also: for time reducing:
        # dataframe self.uploaded_feature_df and
        #
        time_1 = datetime.datetime.now()
        print(time_1.strftime("time_1: %H-%M-%S"))
    ...
    def clicked_single_item(self, item):
        ...
        if self.single_item_clicked in self.any_line:
            ...
            clrs = ['lime' if (x != 0) else 'grey' for x in self.overview_available_features[
                self.overview_available_features['lineshort'] == self.single_item_clicked]['uploaded_ctn']]
    ...
    def print_message(self, str_val):
        ...
        QMessageBox.information(self, "248Done!", str_val)


if __name__ == '__main__':
    app = QApplication(sys.argv)
    window = WindowShowFeatureOverview(200, 200, 100)
    window.show()
    sys.exit(app.exec_())
